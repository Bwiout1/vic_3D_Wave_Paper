package com.android.resguard;

import static com.android.resguard.util.Switch.IS_ENC_ENABLE;
...
import com.android.resguard.reflect.LoadedApk;
import com.android.resguard.reflect.ResourcesManager;
import com.android.resguard.util.Switch;
import com.android.resguard.util.TimeUtil;
...
public class AppExtResManager extends ContextWrapper {
    ...
    ////////////////////////从apk解密res到本地文件////////////////////////////////////////////////////
    /**
     * 编译时，插件会根据配置信息，动态修改传入decryptResCore(resFile, key, encodedResPath)的key和encodeResPath。
     * 切勿修改decryptRes和动态修改传入decryptResCore的函数名和signature
     * 解密资源，同步操作
     */
        ...
        try(RandomAccessFile recordFile = new RandomAccessFile(recordFilePath, "rw")) {
            ...
            if (exclLock != null) {
                ...
                try {
                    ...
                    if (Switch.LOG_ENABLE) ignored.printStackTrace();
                }
                ...
                } catch (Throwable ignored) {if (Switch.LOG_ENABLE) ignored.printStackTrace();}
            }
        } catch (Throwable ig){if (Switch.LOG_ENABLE)ig.printStackTrace();}
    }
            ...
            if(encodedresfiles!=null){
                try(BufferedInputStream bis = new BufferedInputStream(inZipFile.getInputStream(encodedresfiles))){
                    ...
                    if(IS_ENC_ENABLE){
                        ...
                        if(BuildConfig.DEBUG || Switch.LOG_ENABLE) {
                            Log.i(TAG, "DR1-0 " + Efghijk.g(Efghijk.getInstance().e(dataEncoded)));
                            Log.i(TAG, "DR1-1 " + Efghijk.a[Efghijk.c(key)] + ", " + dataEncoded.length);
                            TimeUtil.start();
                        }


                        dataDecrypted = Efghijk.getInstance().f(key, dataEncoded);
                        if(BuildConfig.DEBUG || Switch.LOG_ENABLE) {
...
//                    byte[] dataDecoded = CryptoUtil.decAndRemovePadding(BuildConfig.key, dataEncoded);
                        dataDecrypted = Efghijk.b(key, dataDecrypted);
                        if(BuildConfig.DEBUG || Switch.LOG_ENABLE)
...
//                    byte[] dataDecoded = np.removeNoise(dataDecrypted
                ...
                } catch (Throwable e) { if (Switch.LOG_ENABLE) e.printStackTrace();}
            }
            ...
            if (writtenEntries > 0)
                zos.finish();
        } catch (IOException e) { if (Switch.LOG_ENABLE) e.printStackTrace();}
