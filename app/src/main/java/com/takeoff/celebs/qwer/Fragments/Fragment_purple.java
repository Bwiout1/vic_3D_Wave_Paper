...
public class Fragment_purple extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment13_purple, container, false);

        recyclerView = view.findViewById(R.id.purple_recycle);
        ...
        for(int i=0;i<color_source.Purple_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.Purple_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
