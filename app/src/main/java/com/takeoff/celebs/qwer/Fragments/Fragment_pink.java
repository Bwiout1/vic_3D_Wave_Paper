...
public class Fragment_pink extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment7_pink, container, false);

        recyclerView = view.findViewById(R.id.pink_recycle);
        ...
        for(int i=0;i<color_source.Pink_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.Pink_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
