...
public class Fragment_love extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment5_love, container, false);
        ...
        for(int i=0;i<photoSource.love_source.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(photoSource.love_source[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
