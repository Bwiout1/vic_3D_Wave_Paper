...
public class Fragment_orange extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment9_orange, container, false);

        recyclerView = view.findViewById(R.id.orange_recycle);
        ...
        for(int i=0;i<color_source.Orange_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.Orange_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
