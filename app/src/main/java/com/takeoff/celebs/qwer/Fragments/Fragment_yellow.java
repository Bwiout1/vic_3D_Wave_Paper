...
public class Fragment_yellow  extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment10_yellow, container, false);

        recyclerView = view.findViewById(R.id.yellow_recycle);
        ...
        for(int i=0;i<color_source.Yellow_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.Yellow_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
