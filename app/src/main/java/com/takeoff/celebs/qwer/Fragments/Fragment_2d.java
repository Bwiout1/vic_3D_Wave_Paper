...
import com.takeoff.celebs.qwer.HomeActivity;
...
public class Fragment_2d extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment4_2d, container, false);

        recyclerView = view.findViewById(R.id.parallax_2d_recycle);

        GridLayoutManager layoutManager = new GridLayoutManager(this.getActivity(), 2);


        photoSource = new PhotoSource();
        photoAdapter = new PhotoAdapter(this.getContext());
        recyclerView.setLayoutManager(layoutManager);
        datalist = new ArrayList<>();
        photoAdapter.setData(datalist);
        for(int i=0;i<photoSource.hd_source.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(photoSource.hd_source[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
