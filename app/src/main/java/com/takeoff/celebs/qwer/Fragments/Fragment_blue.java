...
public class Fragment_blue extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment12_blue, container, false);

        recyclerView = view.findViewById(R.id.blue_recycle);
        ...
        for(int i=0;i<color_source.Blue_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.Blue_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
