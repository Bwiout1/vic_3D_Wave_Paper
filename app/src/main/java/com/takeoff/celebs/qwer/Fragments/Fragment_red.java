...
public class Fragment_red extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment8_red, container, false);

        recyclerView = view.findViewById(R.id.red_recycle);
        ...
        for(int i=0;i<color_source.Red_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.Red_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
