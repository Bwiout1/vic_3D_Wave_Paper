...
public class Fragment_gray extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment6_gray, container, false);

        recyclerView = view.findViewById(R.id.gray_recycle);
        ...
        for(int i=0;i<color_source.White_url.length;i++){
            PhotoBean item = new PhotoBean();
            int num = Get_like_num();
            item.setUrl(color_source.White_url[i]);
            item.setName(num +"K");
            datalist.add(item);
        }
